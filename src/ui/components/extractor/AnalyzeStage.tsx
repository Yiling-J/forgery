...
export const AnalyzeStage: React.FC<AnalyzeStageProps> = ({ imageSrc, statusMessage }) => {
  return (
    <div className="relative flex flex-col items-center justify-center w-full h-full overflow-hidden bg-slate-50">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full max-w-4xl h-[70vh] p-8">
        ...
        {/* Status Text */}
        <div className="mt-8 flex items-center space-x-3 px-6 py-3 rounded-full bg-white/80 backdrop-blur-md border border-slate-200 shadow-xl">
      ...
      <style>{`
        @keyframes scanline {
          ...
          100% {
            transform: translateY(100vh); /* Adjust based on container height if needed, but 100vh ensures full sweep */
          }
        }
      `}</style>
    </div>
