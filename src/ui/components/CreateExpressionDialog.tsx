...
export const CreateExpressionDialog: React.FC<CreateExpressionDialogProps> = ({
  open,
  onOpenChange,
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await client.expressions.$post({
        form: {
          name,
          image: file,
        },
      ...
      console.error(err)
      toast.error('Failed to create expression')
    } finally {
      setLoading(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>Upload a reference image for a new expression.</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="expression-image">Expression Image</Label>
            <div className="flex items-center justify-center w-full">
              {!preview ? (
                <label
                  htmlFor="expression-dropzone"
          ...
          <div className="space-y-2">
            <Label htmlFor="expression-name">Name</Label>
            <Input
              id="expression-name"
              value={name}
