     ...
     return (
        <div className="animate-fade-in-up space-y-8">
           ...
           <div className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
              <h3 className="text-xs font-bold text-stone-400 uppercase tracking-widest mb-3">Source Reference</h3>
           ...
           <div className="bg-white p-6 rounded-xl border border-stone-200 shadow-lg">
              <div className="flex justify-between items-center mb-6">
                 <div>
                    ...
                    <p className="text-stone-500 text-sm font-medium">{results.length} items found & stashed</p>
                 </div>
                 ...
                 {results.map((item, i) => (
                    <div key={i} className="group relative bg-stone-50 rounded-lg border-2 border-stone-200 hover:border-amber-400 hover:shadow-lg transition-all duration-300">
                       <div className="aspect-square p-4 flex items-center justify-center relative overflow-hidden rounded-t-lg">
                          ...
                          <img src={item.imageUrl} className="max-w-full max-h-full object-contain drop-shadow-md group-hover:scale-110 transition-transform" />
    ...
    <div className="max-w-2xl mx-auto animate-fade-in-up">
       {!file ? (
          <div className="border-4 border-dashed border-stone-300 bg-white rounded-3xl p-12 text-center hover:border-cyan-400 hover:bg-cyan-50/30 transition-all group">
             ...
             <p className="text-stone-500 mb-4 max-w-sm mx-auto">Drop character image to identify and extract equipment sprites.</p>
             <div className="block">
                <label className="inline-block cursor-pointer">
                    <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                    <span className="px-8 py-4 bg-stone-900 text-white font-bold uppercase tracking-wider rounded-xl shadow-xl hover:bg-stone-800 transition-colors">Select Image</span>
                </label>
             </div>
          </div>
       ) : (
          <div className="bg-white rounded-2xl shadow-xl border border-stone-200 p-6">
             <div className="flex justify-between items-center mb-6 pb-6 border-b border-stone-100">
                ...
                <button onClick={handleReset} className="text-red-500 font-bold text-xs uppercase hover:text-red-700">Remove</button>
             </div>
             ...
             <p className="text-center text-xs text-stone-400 mt-4 font-mono">POWERED BY GEMINI 2.0 FLASH</p>
          </div>
       )}
    </div>
  );
