...
const App: React.FC = () => {
  ...
  const handleSplitConfirm = useCallback((selectedIds: string[]) => {
      ...
      <main className="relative z-10 w-full h-screen flex flex-col">
        ...
        <header className="px-6 py-4 flex items-center justify-between border-b border-white/5 bg-slate-900/50 backdrop-blur-sm">
           <div className="flex items-center gap-2">
             ...
             <h1 className="font-semibold text-lg tracking-tight text-slate-200">Refine<span className="text-sky-400">Lab</span></h1>
           </div>
           
           {/* Progress Steps */}
           {stage !== 'upload' && (
             <div className="hidden sm:flex items-center gap-2 text-xs font-medium bg-black/20 p-1 rounded-full border border-white/5">
                <div className={`px-3 py-1 rounded-full transition-colors ${stage === 'analyze' ? 'bg-sky-500/20 text-sky-400' : 'text-slate-500'}`}>Analyze</div>
                <div className="w-4 h-[1px] bg-slate-800"></div>
                <div className={`px-3 py-1 rounded-full transition-colors ${stage === 'split' ? 'bg-sky-500/20 text-sky-400' : 'text-slate-500'}`}>Split</div>
                <div className="w-4 h-[1px] bg-slate-800"></div>
                <div className={`px-3 py-1 rounded-full transition-colors ${stage === 'refine' ? 'bg-purple-500/20 text-purple-400' : 'text-slate-500'}`}>Refine</div>
             </div>
           )}
        </header>
        ...
        <div className="flex-1 relative overflow-hidden">
          {stage === 'upload' && (
            <UploadStage onImageUpload={handleImageUpload} />
          )}
